...
const MODES: Array<{ id: Mode; label: string; helper: string }> = [
  ...
  { id: "apk", label: "APK", helper: "Paste the app pitch or manifest summary if an APK is involved." },
...
function inferEmergencyTrigger(mode: Mode, flags: {
...
export default function App() {
  ...
  const analyzeLabel = useMemo(() => {
    if (mode === "emergency") return "Stabilize incident";
  ...
  async function onAnalyze() {
    ...
    const payload = {
      ...
      apkFileName: mode === "apk" ? secondaryHint || undefined : undefined,
    };
    ...
    try {
      ...
      setError(
        `${message}. Set API base to the machine running FastAPI, for example ${DEFAULT_API_BASE} on the same device or your LAN IP plus /api on a phone.`,
      );
      setResult(null);
      setEmergency(null);
    } finally {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.eyebrow}>Chetana v2</Text>
        <Text style={styles.title}>Check before you act.</Text>
        <Text style={styles.subtitle}>
          One fraud-interception flow for message, QR, call, job, APK, and emergency recovery.
        </Text>
